...
export default function AddToGlossaryModal({
    ...
    language,
    onGlossaryItemAdded,
}: AddToGlossaryModalProps): ReactElement {
    ...
    const selectedLanguage = () =>
        regionAgnostic ? language.substring(0, 2) : language;
    ...
    const addToGlossary = async () => {
        ...
        try {
            ...
            await Fetch.post(
                `/api/glossarymanager/${addGlossary.id}/${selectedLanguage()}`,
                {
                    term: term,
            ...
            <VerticalLayout>
                <HorizontalLayout>
                    <div className={Styles.termBox}>
                        <LineEdit
                            placeholder={t("ADD_TO_GLOSSARY_TERM")}
                            value={term}
                            onChange={e =>
                ...
                <LineEdit
                    placeholder={t("TRANSLATION_AREA_TITLE", {
                        lang: I18n.humanReadableLocale(selectedLanguage()),
                    })}
                    dir={I18n.dir(selectedLanguage())}
                    value={translation}
                    onChange={e =>
                        setTranslation((e.target as HTMLInputElement).value)
                    }
                />
                {I18n.isRegionAgnostic(language) || (
                    <div>
                        <input
                            id={regionAgnosticCheckboxId}
                            type={"checkbox"}
                            checked={regionAgnostic}
                            onChange={e =>
                                setRegionAgnostic(
                        ...
                        />
                        <label htmlFor={regionAgnosticCheckboxId}>
                            {t("REGION_AGNOSTIC")}
                        </label>
                    </div>
                )}
                {connectedGlossaries.length > 1 && (
                    <>
                        ...
                        <SelectableList
                            items={connectedGlossaries.map(glossary => ({
                                contents: (
                                    <div className={Styles.glossaryItem}>
                                        <Icon
                                            icon={"dialog-ok"}
                                            className={`${Styles.glossaryCheck} ${addGlossary.id === glossary.id && Styles.checked}`}
                                        />
