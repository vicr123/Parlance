...
export function ThreadItem({
    ...
    return (
        <div
            className={`${Styles.threadItem} ${item.isClosed && Styles.closed} ${noPadding && Styles.noPadding}`}
            onClick={() => onCurrentThreadChanged(item)}
        >
            ...
            <div className={Styles.lastMessage}>
                ...
                <span className={Styles.threadCreator}>
                    <img
                        className={Styles.threadCreatorImage}
                        src={item.headComment.author.picture}
                        alt={t("COMMENT_THREAD_PROFILE_PICTURE_ALT_TEXT", {
                            user: item.headComment.author.username,
                        })}
                    />
