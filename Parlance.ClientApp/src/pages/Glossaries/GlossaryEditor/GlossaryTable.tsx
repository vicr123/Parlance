        ...
        <div className={className}>
            <div className={Styles.errorView}>
                <VerticalLayout className={Styles.errorViewInner}>
                    <SilentInformation
                        title={t("No Language Selected")}
                        text={t(
                            "Select a language to view the glossary entries",
                        )}
                    />
                </VerticalLayout>
            </div>
...
export default function GlossaryTable({
    ...
    const onAdd = () => {
        Modal.mount(
            <AddToGlossaryModal
                language={language}
                connectedGlossaries={[glossaryObject]}
    ...
    const onDelete = async (item: GlossaryItem) => {
        ...
        try {
            await Fetch.delete(
                `/api/GlossaryManager/${glossary}/${language}/${item.id}`,
            );
        } catch (e) {
            Modal.mount(
                <ErrorModal
    ...
    const visibleGlossaryItems = searchQuery
        ? glossaryItems.filter(x =>
              x.term.toLowerCase().includes(searchQuery.toLowerCase()),
          )
        : glossaryItems;
                ...
                {visibleGlossaryItems.map(match => (
                    <div key={match.id} className={Styles.match}>
                        ...
                        <span className={Styles.buttons}>
                            {canTranslate && (
                                <SmallButton onClick={() => onDelete(match)}>
                                    {t("DELETE")}
                                </SmallButton>
                            )}
                        </span>
