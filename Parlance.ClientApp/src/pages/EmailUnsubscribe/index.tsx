...
type UnsubscribeOptions =
    | "UnsubscribeOnly"
    | "UnsubscribeTerminateAutoSubscription"
    | "UnsubscribeTotally"
    | "UnverifyEmail";
...
export default function EmailUnsubscribe() {
    ...
    const updateSubscriptionState = async () => {
        const response = await Fetch.post<UnsubscribeInformation>(
            "/api/emailunsubscribe",
            {
                token: token,
            },
        );
    ...
    const unsubscribe = async (type: UnsubscribeOptions) => {
        ...
        try {
            ...
            Modal.mount(
                <Modal
                    heading={t("UNSUBSCRIBE_ERROR_HEADING")}
    ...
    const unsubscribeTotally = () => {
        Modal.mount(
            <Modal
                heading={t("EMAIL_UNSUBSCRIBE_COMPLETELY_HEADER")}
    ...
    const unverifyEmail = () => {
        Modal.mount(
            <Modal
                heading={t("UNSUBSCRIBE_UNVERIFY_CONFIRM_TITLE")}
                buttons={[Modal.CancelButton]}
            >
        ...
        <div>
            <Container>
                ...
                <SelectableList
                    items={[
                        {
                            contents: subscription.autoSubscription
                                ? t("EMAIL_UNSUBSCRIBE_ONLY")
                                : t("EMAIL_DO_UNSUBSCRIBE"),
                            onClick: () => unsubscribe("UnsubscribeOnly"),
                        },
                        subscription.autoSubscription && {
                            contents: (
                                <Trans
                                    i18nKey={"EMAIL_UNSUBSCRIBE_AND_STOP_AUTO"}
                                >
                                    Unsubscribe me and stop automatically
                                    subscribing me when I{" "}
                                    <AutoSubscribeEvent
                                        autoSubscribeEvent={
                                            subscription.autoSubscription
                                        }
            ...
            {emailNotificationsOn && (
                <>
                    ...
                    <Container>
                        ...
                        <SelectableList onClick={unverifyEmail}>
