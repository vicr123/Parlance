...
export default function ThreadReplyArea({
    ...
    const createThread = async () => {
        ...
        if (threadId) {
            try {
                onThreadDataChanged!(
                    await Fetch.post(`/api/comments/${thread.id}`, {
                        body: body,
                    }),
                );
            } catch {
            ...
            try {
                thread = await Fetch.post(
                    `/api/comments/${project}/${subproject}/${language}/${tkey}`,
                    {
                        title: title,
                        body: body,
                    },
    ...
    const toggleClosed = async () => {
        ...
        if (thread.isClosed) {
            try {
                ({ comments, thread: newThread } =
                    await Fetch.delete<CloseResponse>(
                        `/api/comments/${thread.id}/close`,
                    ));
            } catch {
            ...
            try {
                ({ comments, thread: newThread } =
                    await Fetch.post<CloseResponse>(
                        `/api/comments/${thread.id}/close`,
                        null,
                    ));
            } catch {
                ...
                <>
                    ...
                    <div className={Styles.buttonContainer}>
                        <div className={Styles.postingPrompt}>
                            {t("COMMENT_POSTING_AS_PROMPT", {
                                user: UserManager.currentUser!.username,
                            })}
                        </div>
