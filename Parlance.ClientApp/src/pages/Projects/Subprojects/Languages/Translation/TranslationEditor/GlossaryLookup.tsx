...
export default function GlossaryLookup({
    ...
    useEffect(() => {
        ...
        let matches: GlossaryResult[] = glossary.filter(
            item =>
                tokens.some(
                    token =>
                        item.term.toLowerCase() ==
                        (token.lemma || token.normal).toLowerCase(),
                ) ||
                new RegExp(`\\b${item.term}\\b`, "iu").exec(sourceString) !==
                    null,
        );
        matches.push(
            ...tokens
        ...
        setMatches([
            ...new Map(matches.map(item => [item.term, item])).values(),
        ]);
                ...
                {matches.map(match => (
                    <div key={match.id} className={Styles.match}>
                        <span>
                            {match.term}{" "}
                            {match.translation &&
                                `(${t(PartOfSpeechTranslationString(match.partOfSpeech))})`}{" "}
                            ={" "}
                        </span>
                        <span>{match.translation || "?"}</span>
                        <span className={Styles.suggestAddButton}>
                            {match.translation ? (
                                <SmallButton onClick={() => copy(match)}>
                                    {t("COPY")}
                                </SmallButton>
                            ) : (
                                canEdit && (
                                    <SmallButton
                                        onClick={() => addToGlossary(match)}
                                    >
                                        {t("ADD_TO_GLOSSARY")}
                                    </SmallButton>
                                )
