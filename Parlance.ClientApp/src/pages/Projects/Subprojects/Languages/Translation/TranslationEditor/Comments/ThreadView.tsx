...
function ThreadComment({ data }: { data: Comment }) {
    return (
...
function ThreadEvent({ data }: { data: Comment }) {
    return (
        <div className={Styles.eventItem}>
            ...
            <span className={Styles.authorDetails}>
                ...
                <EventName event={data.event!} /> {moment(data.date).fromNow()}
            </span>
        </div>
    );
}
...
function ThreadHeader({ thread }: { thread: Thread }) {
    ...
    return (
        <div className={Styles.headerItem}>
            <VerticalLayout>
                <PageHeading level={4}>
                    {t("COMMENT_ORIGINAL_TRANSLATION")}
                </PageHeading>
                <div dir={I18n.dir(thread.language)}>
                ...
                {!!thread.sourceTranslation && (
                    <HorizontalLayout>
                        <SmallButton onClick={goToString}>
                            {t("COMMENT_GO_TO_STRING")}
                        </SmallButton>
                    </HorizontalLayout>
                )}
            </VerticalLayout>
...
export default function ThreadView({
    ...
    return (
        <VerticalLayout gap={0}>
            ...
            {threadData.map((x, i) =>
                x.event ? (
                    <ThreadEvent key={i} data={x} />
                ) : (
                    <ThreadComment key={i} data={x} />
                ),
            )}
            <ThreadReplyArea
                thread={thread}
                onThreadDataChanged={setThreadData}
