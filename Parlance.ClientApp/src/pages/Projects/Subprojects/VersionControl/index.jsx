...
function Commit({ commit }) {
    if (!commit) {
        return (
            <div className={Styles.commitContainer}>
                <PreloadingBlock>
                    COMMIT MESSAGE PLACEHOLDER ITEMS
                </PreloadingBlock>
                <PreloadingBlock width={"auto"}>
                    <code>0000000</code>
                </PreloadingBlock>
            </div>
        );
    ...
    return (
        <div className={Styles.commitContainer}>
            ...
            <code className={Styles.commitIdentifier}>
                {commit.commitIdentifier.substring(0, 7)}
            </code>
        </div>
    );
}
...
export default function VersionControl() {
            ...
            Modal.mount(
                <ErrorModal
                    ...
                    specialRenderings={{
                        DirtyWorkingTree: (
                            <Modal
                                buttons={[
                                    ...
                                    {
                                        text: t(
                                            "VCS_DISCARD_UNCOMMITTED_CHANGES",
                                        ),
                        ...
                        MergeConflict: (
                            <Modal
                                heading={t("Merge Conflict")}
                                buttons={[Modal.OkButton]}
                            >
                                <VerticalLayout>
                                    <span>
                                        {t("translation:ERROR_MERGE_CONFLICT")}
                                    </span>
                                    <VerticalSpacer height={20} />
                                    <PageHeading level={3}>
                                        {t("Reconciling Changes")}
                                    </PageHeading>
                                    <span>
                                        {t(
                                            "In order to reconcile the repository, merge the Parlance repository locally by adding the below remote and merging it:",
                                        )}
                                    </span>
                                    <code>{cloneUrl}</code>
                                </VerticalLayout>
    ...
    const discardUncommittedChanges = () => {
        Modal.mount(
            <Modal
                heading={t("VCS_DISCARD_UNCOMMITTED_CHANGES")}
                buttons={[
                    Modal.CancelButton,
                    {
                        text: t("VCS_DISCARD"),
                        onClick: async () => {
                            ...
                            try {
                                await Fetch.delete(
                                    `/api/projects/${project}/vcs/uncommitted`,
                                );
                                await updateVcs();
                                Modal.unmount();
                            } catch (err) {
            ...
            <ErrorCover error={error}>
                <Container>
                    ...
                    <div className={Styles.infoGrid}>
                        ...
                        <div className={Styles.buttonBox}>
                            <SmallButton onClick={pull}>
                                {t("VCS_INCOMING_COMMITS_PULL", {
                                    count: vcsState?.behind || 0,
                                })}
                            </SmallButton>
                        </div>
                        ...
                        <div className={Styles.buttonBox}>
                            <SmallButton onClick={commit}>
                                {t("VCS_UNCOMMITTED_CHANGES_COMMIT", {
                                    count: vcsState?.changedFiles.length || 0,
                                })}
                            </SmallButton>
                        </div>
                        ...
                        <div className={Styles.buttonBox}>
                            <SmallButton onClick={push}>
                                {t("VCS_OUTGOING_COMMITS_PUSH", {
                                    count: vcsState?.ahead || 0,
                                })}
                            </SmallButton>
                        </div>
                        ...
                        <div className={Styles.buttonBox}>
                            <code>{cloneUrl}</code>
                            <SmallButton onClick={copyCloneUrl}>
                                {t("Copy")}
                            </SmallButton>
                        </div>
                    </div>
