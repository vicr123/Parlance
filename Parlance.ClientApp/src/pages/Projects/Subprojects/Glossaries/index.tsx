...
function GlossaryList({
    ...
    shiftGlossary,
    onLeft,
}: GlossaryListProps): ReactElement {
    ...
    if (glossaries === null) {
        ...
        list = (
            <SelectableList
                items={glossaries?.map(x => ({
                    contents: (
                        <div className={Styles.glossaryItem}>
                            <span>{x.name}</span>
                            <Icon
                                className={Styles.glossaryItemIcon}
                                icon={onLeft ? "go-next" : "go-previous"}
                                flip={true}
                            />
                        </div>
                    ),
    ...
    return (
        <div className={`${Styles.listRoot} ${onLeft ? Styles.left : ""}`}>
            <PageHeading className={Styles.listHeading} level={3}>
                {title}
            </PageHeading>
            {list}
        </div>
...
function GlossaryManager(): ReactElement {
    ...
    const [connectedGlossaries, setConnectedGlossaries] = useState<
        Glossary[] | null
    >(null);
    const [disconnectedGlossaries, setDisconnectedGlossaries] = useState<
        Glossary[] | null
    >(null);
    ...
    useEffect(() => {
        (async () => {
            ...
            setDisconnectedGlossaries(
                glossaries.filter(
                    x => !connectedGlossaries.some(y => x.id === y.id),
                ),
            );
        })();
    }, []);
    ...
    const connectGlossary = async (glossary: Glossary) => {
        setConnectedGlossaries([
            ...(connectedGlossaries as Glossary[]),
            glossary,
        ]);

        let disconnected = [...(disconnectedGlossaries as Glossary[])];
        disconnected.splice(
            disconnectedGlossaries!.findIndex(x => x.id == glossary.id),
            1,
        );
    ...
    const disconnectGlossary = async (glossary: Glossary) => {
        setDisconnectedGlossaries([
            ...(disconnectedGlossaries as Glossary[]),
            glossary,
        ]);

        let connected = [...(connectedGlossaries as Glossary[])];
        connected.splice(
            connectedGlossaries!.findIndex(x => x.id == glossary.id),
            1,
        );
        ...
        try {
            await Fetch.delete(
                `/api/projects/${project}/glossary/${glossary.id}`,
            );
        } catch (e) {
            Modal.mount(
                <ErrorModal
    ...
    return (
        <>
            <div
                className={`${Styles.glossaryManagerRootContainer} ${mobileSwitch && Styles.mobileSwitch}`}
            >
                <div className={Styles.glossaryManagerRoot}>
                    <VerticalLayout className={Styles.glossaryManagerSide}>
                        <GlossaryList
                            title={t("CONNECTED_GLOSSARIES")}
                            glossaries={connectedGlossaries}
                            shiftGlossary={disconnectGlossary}
                            onLeft={true}
                        />
                        <div className={Styles.mobileSwitcher}>
                            <SelectableList
                                onClick={() => setMobileSwitch(true)}
                            >
                    ...
                    <VerticalLayout className={Styles.glossaryManagerSide}>
                        <GlossaryList
                            title={t("AVAILABLE_GLOSSARIES")}
                            glossaries={disconnectedGlossaries}
                            shiftGlossary={connectGlossary}
                            onLeft={false}
                        />
                        <div className={Styles.mobileSwitcher}>
                            <SelectableList
                                onClick={() => setMobileSwitch(false)}
                            >
