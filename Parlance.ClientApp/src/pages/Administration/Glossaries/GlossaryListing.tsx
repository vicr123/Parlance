...
function AddGlossaryModal({ onDone }: AddGlossaryModalProps): ReactElement {
    ...
    return (
        ...
        >
            ...
            <LineEdit
                placeholder={t("GLOSSARY_ADD_GLOSSARY_NAME")}
                value={glossaryName}
                onChange={e =>
                    setGlossaryName((e.target as HTMLInputElement).value)
                }
            />
        </Modal>
    );
}
function RemoveGlossaryModal({
    onDone,
    glossary,
}: RemoveGlossaryModalProps): ReactElement {
    const { t } = useTranslation();
        ...
        <div>
            <ListPageBlock>
                <VerticalLayout>
                        ...
                        items={glossaries.map(glossary => ({
                            contents: (
                                <div className={Styles.glossaryItem}>
                                    ...
                                    <span
                                        className={Styles.glossaryItemSubtext}
                                    >
                                        {t("ADD_GLOSSARY_PROJECTS_CONNECTED", {
                    ...
                    />
                    <SelectableList onClick={addGlossary}>
                        {t("ADD_GLOSSARY")}
                    </SelectableList>
