        ...
        <VerticalLayout>
            <SelectableList
                items={[
                    ...
                    ...keys.map(key => ({
                        contents:
                            key.application === "Parlance" ? (
                                key.name
                            ) : (
                                <div className={Styles.SecurityKey}>
                                    <span className={Styles.SecurityKeyName}>
                                        {key.name}
                                    </span>
                                    <span
                                        className={
                                            Styles.SecurityKeyApplication
                                        }
                                    >
                                        {key.application}
                                    </span>
...
function SecurityKeysUi({
    ...
    const manageKey = (key: SecurityKey) => {
        Modal.mount(
            <Modal
                heading={t("SECURITY_KEY_DEREGISTER")}
                buttons={[
                    Modal.CancelButton,
                    {
    ...
    const registerKey = (type: string) => {
        ...
        if (!window.PublicKeyCredential) {
            Modal.mount(
                <Modal
                    heading={t("translation:UNSUPPORTED_BROWSER")}
    ...
    return (
        <>
            <Container>
                <VerticalLayout>
                    <PageHeading level={3}>
                        {t("ACCOUNT_SETTINGS_MANAGE_SECURITY_KEYS")}
                    </PageHeading>
                    <span>
                        {t("ACCOUNT_SETTINGS_MANAGE_SECURITY_KEYS_PROMPT")}
                    </span>
                </VerticalLayout>
            </Container>
            {keys.filter(key => key.application === "Parlance").length > 0 && (
                <Container>
                    {
                        <KeyList
                            keys={keys.filter(
                                key => key.application === "Parlance",
                            )}
                ...
                <Container>
                    {
                        <KeyList
                            ...
                            after={t(
                                "translation:SECURITY_KEY_OTHER_SECURITY_KEYS_PROMPT",
...
export default function SecurityKeys() {
    ...
    const updateState = async () => {
        ...
        try {
            setSecurityKeyState(
                await Fetch.post("/api/user/keys", {
                    password: password,
                }),
            );
            Modal.unmount();
    ...
    if (securityKeyState === null) {
        ...
        content = (
            <SecurityKeysUi
                password={password}
                onUpdateKeys={updateState}
                keys={securityKeyState}
            />
        );
    }
