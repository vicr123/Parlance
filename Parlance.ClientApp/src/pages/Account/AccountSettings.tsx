...
export default function AccountSettings() {
    ...
    let verifyEmailPrompt = UserManager.currentUser.emailVerified ? null : (
        <div className={Styles.verifyEmailPrompt}>
            <PageHeading level={3}>{t("EMAIL_VERIFY_TITLE")}</PageHeading>
            <p>{t("EMAIL_VERIFY_PROMPT")}</p>
            <div className={Styles.verifyEmailButtons}>
                <SmallButton onClick={resendVerificationEmail}>
                    {t("EMAIL_VERIFY_RESEND")}
                </SmallButton>
                <SmallButton onClick={() => navigate("verify")}>
                    {t("EMAIL_VERIFY_ENTER_CODE")}
                </SmallButton>
            </div>
        </div>
    );
            ...
            <Container style={{ backgroundColor: "unset" }}>
                <div className={Styles.accountHeader}>
                    ...
                    <img
                        ...
                        src={UserManager.currentUserProfilePicture}
                    />
                    {verifyEmailPrompt}
                </div>
            </Container>
            <Container>
                <SelectableList
                    items={[
                        ...
                        {
                            contents: t(
                                "ACCOUNT_SETTINGS_CHANGE_PROFILE_PICTURE",
                            ),
                            onClick: () =>
                                window.open(
                                    "https://en.gravatar.com/gravatars/new/",
                                    "_blank",
                                ),
                        },
                        {
                            contents: t(
                                "ACCOUNT_SETTINGS_CHANGE_EMAIL_ADDRESS",
                            ),
