...
export default function Attribution() {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const consentDetails = await Fetch.get("/api/user/attribution/consent");
    ...
    return <div>
        ...
        <Container>
            <VerticalLayout>
                ...
                <div>
                    <input type={"checkbox"} id={checkboxId} checked={haveConsent}
                           onChange={e => setHaveConsent(e.target.checked)}/>
                    <label htmlFor={checkboxId}>{t("ATTRIBUTION_CHECKBOX")}</label>
                </div>
                {haveConsent && <>
                    <hr/>
                    <span>{t("ATTRIBUTION_PROMPT_3")}</span>
                    <LineEdit placeholder={t("PREFERRED_NAME")} value={preferredName}
                              onChange={e => setPreferredName(e.target.value)}/>
                </>}
                <VerticalSpacer height={20}/>
                <SelectableList
                    onClick={applySettings}>{t("APPLY_ATTRIBUTION_SETTINGS")}</SelectableList>
            </VerticalLayout>
        </Container>
    </div>
}