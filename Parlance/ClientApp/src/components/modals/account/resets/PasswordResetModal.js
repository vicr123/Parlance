...
export default function PasswordResetModal(props) {
    ...
    return <Modal heading={t('PASSWORD_RECOVERY_TITLE')} buttons={[
        {
            text: t('CANCEL'),
        ...
        <ModalList>
            {props.resetMethods.map(method => {
                switch (method.type) {
                    case "email":
                        return {
                            text: t("PASSWORD_RECOVERY_EMAIL", {email: `${method.user}∙∙∙@${method.domain}∙∙∙`}),
                            onClick: () => Modal.mount(<EmailResetModal resetMethods={props.resetMethods}
                                                                        method={method}/>)
                        }
                    default:
                        return null;
